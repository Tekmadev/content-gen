...
const APP_URL =
  process.env.NEXT_PUBLIC_APP_URL ?? "https://content.tekmadev.com";
...
export async function GET(request: Request) {
  ...
  if (error) {
    console.error(
      "[canva/callback] OAuth error:",
      error,
      searchParams.get("error_description"),
    );
    return NextResponse.redirect(
      `${APP_URL}/carousel?canva_error=${encodeURIComponent(error)}`,
    );
  }
  ...
  if (!storedState || storedState !== returnedState) {
    return NextResponse.redirect(
      `${APP_URL}/carousel?canva_error=invalid_state`,
    );
  }

  if (!codeVerifier) {
    return NextResponse.redirect(
      `${APP_URL}/carousel?canva_error=missing_verifier`,
    );
  }
  ...
  if (!clientId || !clientSecret) {
    return NextResponse.redirect(
      `${APP_URL}/carousel?canva_error=missing_credentials`,
    );
  }
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return NextResponse.redirect(
        `${APP_URL}/carousel?canva_error=token_exchange_failed`,
      );
    }
    ...
    await adminSupabase.from("canva_tokens").upsert(
      {
        user_id: user.id,
        access_token,
